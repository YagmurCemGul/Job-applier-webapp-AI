...
class IndeedHandler {
  ...
  private async runApply(command: ContentCommand) {
    ...
    try {
      ...
      this.log(requestId, `Filled ${filled} fields on Indeed`);

      // Indeed file upload
      if (files && files.length > 0) {
        const resumeInput = document.querySelector('input[type="file"][id*="resume"]') as HTMLInputElement;
        if (resumeInput) {
      ...
      if (dryRun) {
        ...
        this.sendResponse(requestId, {
          ...
          payload: {
            ok: true,
            reviewNeeded: true,
            message: 'Indeed dry-run complete',
      ...
      if (submitBtn) {
        submitBtn.click();
        this.log(requestId, 'Clicked Indeed submit');
        ...
        this.sendResponse(requestId, {
          ...
          payload: {
            ok: false,
            reviewNeeded: true,
            message: 'Indeed submit button not found',
            hints: this.autofiller.getLogs(),
          },
          meta: { requestId },
        });
  ...
  private async parsePage(command: ContentCommand) {
    ...
    try {
      const job = PageParser.parse('indeed');
