...
class LeverHandler {
  ...
  async init() {
    ...
    console.log('Lever handler initialized');
  }
  ...
  private async runApply(command: ContentCommand) {
    ...
    try {
      ...
      // Lever may have multi-step forms
  ...
  private async parsePage(command: ContentCommand) {
    ...
    try {
      const job = PageParser.parse('lever');
  ...
  private buildFieldMappings(answers: Record<string, any>): FieldMapping[] {
    ...
    const fieldMap: Record<string, { name?: string; label?: string }> = {
      name: { name: 'name' },
      email: { name: 'email' },
      phone: { name: 'phone' },
      org: { name: 'org' },
