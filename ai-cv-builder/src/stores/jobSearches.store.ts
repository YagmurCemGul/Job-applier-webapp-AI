/**
 * Job Searches Store
 * Step 32 - Manages saved searches and alerts
 */
...
interface JobSearchesState {
  ...
  upsert: (s: Omit<SavedSearch, 'id' | 'createdAt' | 'updatedAt'> & { id?: string }) => string;
  remove: (id: string) => void;
  getById: (id: string) => SavedSearch | undefined;
}

function rid() { 
  return crypto?.randomUUID?.() ?? `s_${Math.random().toString(36).slice(2)}`; 
}
