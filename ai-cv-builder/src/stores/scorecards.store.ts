/**
 * Scorecards Store
 * Manages scorecard templates and submissions with Zustand persistence
 */
...
export const useScorecards = create<ScorecardsState>()(
  persist(
    (set, get) => ({
      templates: [],
      submissions: [],
      upsertTemplate: (t) => set({ templates: [t, ...get().templates.filter(x => x.id !== t.id)] }),
      upsertSubmission: (s) => set({ submissions: [s, ...get().submissions.filter(x => x.id !== s.id)] }),
      byInterview: (interviewId) => get().submissions.filter(s => s.interviewId === interviewId),
      getTemplate: (id) => get().templates.find(t => t.id === id),
    }),
    { name: 'scorecards', storage: createJSONStorage(() => localStorage), version: 1 }
  )
);
