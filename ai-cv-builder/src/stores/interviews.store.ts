/**
 * Interviews Store
 * Manages interview state with Zustand persistence
 */
...
export const useInterviews = create<InterviewsState>()(
  persist(
    (set, get) => ({
      ...
      setStage: (id, stage) => set({ items: get().items.map(x => x.id === id ? ({ ...x, stage, updatedAt: new Date().toISOString() }) : x) }),
      byApplication: (applicationId) => get().items.filter(x => x.applicationId === applicationId),
      getById: (id) => get().items.find(x => x.id === id),
    }),
    { name: 'interviews', storage: createJSONStorage(() => localStorage), version: 1 }
  )
);
