/**
 * Prompt Library Store - Step 30
 * Manages reusable prompts and folders
 */
 ...
 */
export const usePromptLibrary = create<PromptState>()(
  persist(
    (set, get) => ({
      folders: [],
      prompts: [],
      ...
      upsertFolder: (name, id) => {
        ...
        set({
          folders: [doc, ...get().folders.filter((f) => f.id !== fid)],
        })
        return fid
      },
      ...
      upsertPrompt: (p) => {
        ...
        set({
          prompts: [doc, ...get().prompts.filter((x) => x.id !== id)],
        })
        return id
      },
      ...
      listByFolder: (folderId) =>
        get().prompts.filter(
          (p) => p.folderId === folderId || (!folderId && !p.folderId)
        ),
    }),
    {
      name: 'prompt-library',
