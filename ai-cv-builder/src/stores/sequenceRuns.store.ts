/**
 * Sequence runs store for managing active outreach sequences
 */
...
interface RunsState {
  ...
  getById: (id: string) => SequenceRun | undefined;
}

export const useSequenceRuns = create<RunsState>()(
  persist(
    (set, get) => ({
      runs: [],
      ...
      markStep: (id, entry, nextSendAt, nextIndex) => set({
        runs: get().runs.map(r => 
          r.id === id
            ? ({ 
                ...r, 
    ...
    { name: 'sequence-runs', storage: createJSONStorage(() => localStorage), version: 1 }
  )
);
