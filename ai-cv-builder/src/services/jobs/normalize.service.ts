 ...
 * Step 32 - Converts raw job data to normalized format
 */

import type { JobRaw, JobNormalized, SalaryRange, Seniority, EmploymentType } from '@/types/jobs.types';

/**
 * Normalize array of raw jobs
 */
...
export function extractSalary(text: string): SalaryRange | undefined {
  const m = text.match(/(\$|€|£|₺)?\s?(\d{2,3}(?:[.,]\d{3})*|\d+(?:\.\d+)?)[KkMm]?\s*-\s*(\d{2,3}(?:[.,]\d{3})*|\d+(?:\.\d+)?)[KkMm]?/);
  if (!m) return undefined;
  
  const currency = toCurrency(m[1]);
...
/**
 * Classify seniority and employment type from text
 */
export function classify(text: string): { seniority?: Seniority; employment?: EmploymentType } {
  const t = text.toLowerCase();
  
  const seniority: Seniority | undefined =
    /intern/.test(t) ? 'intern' :
    /junior|entry/.test(t) ? 'junior' :
    /senior|sr\./.test(t) ? 'senior' :
    /lead|principal/.test(t) ? 'lead' :
    /manager|head/.test(t) ? 'manager' : 'unspecified';
  
  const employment: EmploymentType | undefined =
    /full[-\s]?time/.test(t) ? 'full-time' :
    /part[-\s]?time/.test(t) ? 'part-time' :
    /contract|freelance/.test(t) ? 'contract' :
    /intern/.test(t) ? 'internship' :
    undefined;
...
// Helper functions
function toCurrency(sym?: string): string | undefined {
  return sym === '₺' ? 'TRY' : 
         sym === '$' ? 'USD' : 
         sym === '€' ? 'EUR' : 
         sym === '£' ? 'GBP' : 
...
function toNumber(s: string): number {
  ...
  return /k$/i.test(s) ? Number(n) * 1000 : 
         /m$/i.test(s) ? Number(n) * 1_000_000 : 
         Number(n);
}
