/**
 * Kariyer.net HTML Adapter
 * Step 32 - Conservative HTML parser for user-provided Kariyer.net content
 * 
...
/**
 * Parse Kariyer.net HTML for job postings
...
export async function fetchKariyerNetHTML(source: SourceConfig): Promise<JobRaw[]> {
  ...
  if (!html) {
    console.warn('No HTML content provided for Kariyer.net HTML adapter');
    return [];
  }
  
  // Conservative parser: extract job links
  const matches = Array.from(
    html.matchAll(/<a[^>]+href="([^"]*\/is-ilani\/[^"]+)"[^>]*>(.*?)<\/a>/gi)
  );
  
  return matches.map((m, i) => ({
    id: `${m[1]}#${i}`,
    url: m[1].startsWith('http') ? m[1] : `https://www.kariyer.net${m[1]}`,
