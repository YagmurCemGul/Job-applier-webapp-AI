...
class JobService {
  ...
  private extractRequirements(text: string): string[] {
    ...
    const reqSection = text.match(/(?:requirements|qualifications|required skills).*?(?=\n\n|\n[A-Z]|$)/is)
    
    if (reqSection) {
      // Extract bullet points or numbered items
      const items = reqSection[0].match(/[-•*]\s*(.+)|^\d+\.\s*(.+)/gm)
      if (items) {
        requirements.push(...items.map(item => item.replace(/^[-•*\d.]\s*/, '').trim()))
      }
    }
  ...
  private extractResponsibilities(text: string): string[] {
    ...
    const respSection = text.match(/(?:responsibilities|duties|you will).*?(?=\n\n|\n[A-Z]|$)/is)
    
    if (respSection) {
      const items = respSection[0].match(/[-•*]\s*(.+)|^\d+\.\s*(.+)/gm)
      if (items) {
        responsibilities.push(...items.map(item => item.replace(/^[-•*\d.]\s*/, '').trim()))
      }
    }
  ...
  private extractSkills(text: string): string[] {
    ...
    const commonSkills = [
      ...
      'C++', 'C#', 'SQL', 'MongoDB', 'PostgreSQL', 'AWS', 'Azure', 'GCP',
      'Docker', 'Kubernetes', 'Git', 'CI/CD', 'Agile', 'Scrum',
      'REST API', 'GraphQL', 'HTML', 'CSS', 'Tailwind', 'Next.js',
      'Vue.js', 'Angular', 'Spring Boot', 'Django', 'Flask', 'Ruby on Rails',
    ]
  ...
  private extractKeywords(text: string): string[] {
    ...
    const stopWords = new Set([
      ...
      'your', 'they', 'been', 'were', 'what', 'when', 'where', 'which',
      'their', 'would', 'there', 'could', 'should', 'about', 'other',
    ])
