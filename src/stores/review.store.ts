/**
 * @fileoverview Zustand store for review cycles, impact tracking, and self-reviews
 */
...
export const useReviews = create<ReviewState>()(
  persist(
    (set, get) => ({
      ...
      upsertCycle: (c) => set({
        cycles: [c, ...get().cycles.filter(x => x.id !== c.id)]
      }),
      
      updateCycle: (id, patch) => set({
      ...
      setImpact: (id, patch) => set({
        impacts: get().impacts.map(i =>
          i.id === id ? { ...i, ...patch } : i
        )
      }),
      
      upsertSelfReview: (s) => set({
