/**
 * @fileoverview Zustand store for 1:1 series and entries.
 * @module stores/oneonones
 */
...
export const useOneOnOnes = create<OneOnOneState>()(
  persist(
    (set, get) => ({
      series: [],
      entries: [],
      upsertSeries: (s) =>
        set({
          series: [s, ...get().series.filter((x) => x.id !== s.id)],
        }),
      upsertEntry: (e) =>
        set({
          entries: [e, ...get().entries.filter((x) => x.id !== e.id)],
        }),
      byPlan: (planId) => ({
        series: get().series.filter((s) => s.planId === planId),
        entries: get().entries.filter(
          (e) => get().series.find((s) => s.id === e.seriesId)?.planId === planId
        ),
      }),
    }),
    {
