...
/**
 * OpenAI provider with graceful fallback
 */
...
function localFallback(provider: string, model: string, req: AIRequest): AIResponse {
  ...
  if (req.task === 'embed' && req.texts) {
    ...
    return { ok: true, provider, model, embeddings, usage: tok(0, 0) }
  }
  ...
  if (req.task === 'moderate' && req.contentToCheck) {
    ...
    return {
      ...
      provider,
      model,
      moderated: { allowed: !bad, flags: bad ? ['heuristic'] : [] },
      usage: tok(0, 0),
    }
  }
