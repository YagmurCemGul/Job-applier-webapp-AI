...
// Supports: OpenAI DALL-E, Replicate (Stable Diffusion), Stability AI
...
class AIImageService {
  ...
  private async generateWithOpenAI(imageBase64: string, prompt: string): Promise<AIImageResult[]> {
    try {
      ...
      if (!result.ok) {
        ...
        throw new Error(error.error?.message || `OpenAI API error: ${result.statusText}`)
      }
  ...
  private async generateWithStability(
    ...
    try {
      ...
      if (!result.ok) {
        const error = await result.json()
        throw new Error(error.message || `Stability API error: ${result.statusText}`)
      }
  ...
  async generateImage(options: AIImageGenerationOptions): Promise<AIImageResult[]> {
    const { prompt, negativePrompt = '', numImages = 1 } = options
