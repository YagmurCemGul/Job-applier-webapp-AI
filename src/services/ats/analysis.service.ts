...
/**
 * Analyze CV against parsed job posting
...
function matchKeywords(cv: CVData, job: ParsedJob) {
  const cvText = normalizeText(stringifyCV(cv))
  const jobKeywords = Array.from(new Set((job.keywords ?? []).map(normalizeText).filter(Boolean)))
...
function sectionSuggestions(cv: CVData): ATSSuggestion[] {
  ...
  if (!cv.summary || cv.summary.trim().length < 40) {
    out.push({
      ...
      action: {
        type: 'add',
        payload: { text: ' Experienced professional with proven track record.' },
      },
      applied: false,
    })
  }
...
function contactSuggestions(cv: CVData): ATSSuggestion[] {
  ...
  if (missing.length) {
    out.push({
      ...
      detail: 'Add complete contact details for recruiters.',
      applied: false,
    })
  }
  return out
...
function experienceEducationSuggestions(cv: CVData): ATSSuggestion[] {
  const out: ATSSuggestion[] = []
...
function stringifyCV(cv: CVData) {
  const skills = (cv.skills ?? []).map((s) => (typeof s === 'string' ? s : s.name)).join(' ')
...
function scoreFrom(matched: number, missing: number, cv: CVData, suggestions: ATSSuggestion[]) {
