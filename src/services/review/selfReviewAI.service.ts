...
/**
 * Generate a self-review grounded in impact entries
 * using STAR and strong verbs
 */
export async function generateSelfReview(
  ...
  try {
    ...
    const text = result.ok && result.text ? String(result.text) : ''
    return materializeSelfReview(cycleId, lang, text)
  } catch {
    return materializeSelfReview(cycleId, lang, '')
  }
}
