...
import { Cloud, Loader2, RefreshCw, CheckCircle, AlertCircle, CloudOff } from 'lucide-react'
      ...
      <TooltipProvider>
        <Tooltip>
          <TooltipTrigger asChild>
            <div
              className={`flex cursor-pointer items-center gap-2 ${status.color}`}
              onClick={() => setShowDetails(!showDetails)}
            >
              {status.icon}
              <span className="hidden text-sm font-medium md:inline">{status.label}</span>
            </div>
          </TooltipTrigger>
          <TooltipContent>
