      ...
      <Dialog open={open} onOpenChange={setOpen}>
        ...
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>Organize your custom prompts into folders</DialogDescription>
          </DialogHeader>

          <div className="space-y-4">
            {/* Create/Edit Folder */}
            <div className="space-y-3 rounded-lg border p-4">
              <h4 className="font-medium">{editingId ? 'Edit Folder' : 'Create New Folder'}</h4>
                      ...
                      {FOLDER_COLORS.map((c) => (
                        <SelectItem key={c.value} value={c.value}>
                          <div className="flex items-center gap-2">
                            <div className={`h-4 w-4 rounded ${c.class}`} />
                            {c.label}
                          </div>
                        </SelectItem>
                      ))}
            ...
            {/* Existing Folders */}
            <div>
              <h4 className="mb-3 font-medium">Existing Folders</h4>
              {folders.length === 0 ? (
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex gap-1">
                          <Button variant="ghost" size="sm" onClick={() => handleEdit(folder.id)}>
                            <Edit2 className="h-4 w-4" />
                          </Button>
                          <Button variant="ghost" size="sm" onClick={() => setDeleteId(folder.id)}>
      ...
      <AlertDialog open={deleteId !== null} onOpenChange={() => setDeleteId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will remove the folder. Prompts in this folder will not be deleted, but will be
              moved to "No Folder".
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
